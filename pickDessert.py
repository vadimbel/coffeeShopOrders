...
class PickDessert(Toplevel):
    def __init__(self):
        Toplevel.__init__(self)
        self.geometry("650x650+650+200")
        self.title("Dessert Menu")
        ...
        # create left frame and place a list with all the dessert_kinds in the menu .
        left_frame = Frame(self, width=200, height=70, relief=SUNKEN, borderwidth=3)
        left_frame.pack(side=LEFT, fill=Y)

        self.dessert_list = Listbox(left_frame, width=35, height=30)
        self.dessert_list.pack()

        # create right frame that will display all the info about every dessert selected in the list box on the left frame .
        ...
        self.display_dessert()

        def display_values(evt):
            """
            This function will be activated when the user click on one of the items in the 'dessert_list' box .
            :param evt:
            :return: None
            """
            # solve a bug : when i click on 'dessert_list' then on 'list_detail' -> exception .
            ...
            dessert = str(self.dessert_list.get(self.dessert_list.curselection()))
            # substring the name out of the selected item .
            dessert_name = dessert[3:]
            # get all the properties of the selected item from db in format : [( ....)] -> tuple inside a list .
            elements = cur.execute("SELECT * FROM dessert_kinds WHERE dessert_name=?", (dessert_name,)).fetchall()
        ...
        # when user click on item in the 'dessert_list' box -> the details will appear in the 'list_detail' .
        self.dessert_list.bind('<<ListboxSelect>>', display_values)

    def display_dessert(self):
        """
        This function created to display the name of the dessert on the 'dessert_list' box .
        :return: None
        """
        # get all the elements from 'cold_drink_kinds' table in db .
        dessert_kinds = cur.execute("SELECT * FROM dessert_kinds").fetchall()

        # insert the name of every dessert to 'dessert_list' box .
        count = 0
        for dessert in dessert_kinds:
            self.dessert_list.insert(count, "{}. {}".format(count+1, str(dessert[0])))
            count += 1

    def add_item_to_check(self):
        """
        This function will be activated when the user select one of the items in the 'dessert_list' box then click the
        'add' button .
        ...
        if len(self.list_detail.curselection()) > 0:
            return
        # the code under will be executed only if the user select one of the items in the 'dessert_list' then click 'add' btn.
        if len(self.dessert_list.curselection()) > 0:
            try:
                # if the user select one of the item in 'dessert_list' and clicked 'add' then :
                ...
                elements = cur.execute("SELECT * FROM dessert_kinds WHERE dessert_name=?", (dessert_name,)) \
                    .fetchall()
