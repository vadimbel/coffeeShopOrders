...
class PickHotDrink(Toplevel):
    def __init__(self):
        Toplevel.__init__(self)
        self.geometry("650x650+650+200")
        self.title("Hot Drink Menu")
        ...
        # create left frame and place a list with all the hot_drink_kinds in the menu .
        left_frame = Frame(self, width=200, height=70, relief=SUNKEN, borderwidth=3)
        left_frame.pack(side=LEFT, fill=Y)

        self.hot_drink_list = Listbox(left_frame, width=35, height=30)
        self.hot_drink_list.pack()

        # create right frame that will display all the info about every hot_drink selected in the list box on the left frame .
        ...
        self.display_hot_drink()

        def display_values(evt):
            """
            This function will be activated when the user click on one of the items in the 'hot_drink_list' box .
            :param evt:
            :return: None
            """
            # solve a bug : when i click on 'hot_drink_list' then on 'list_detail' -> exception .
            if len(self.list_detail.curselection()) > 0:
                return
            # delete all elements from 'list_detail' .
            self.list_detail.delete(0, END)
            # get the selected item from the 'hot_drink_list' box .
            hot_drink = str(self.hot_drink_list.get(self.hot_drink_list.curselection()))
            # substring the name out of the selected item .
            hot_drink_name = hot_drink[3:]
            # get all the properties of the selected item from db in format : [( ....)] -> tuple inside a list .
            elements = cur.execute("SELECT * FROM hot_drink_kinds WHERE hot_drink_name=?", (hot_drink_name,)).fetchall()
        ...
        # when user click on item in the 'hot_drink_list' box -> the details will appear in the 'list_detail' .
        self.hot_drink_list.bind('<<ListboxSelect>>', display_values)

    def display_hot_drink(self):
        """
        This function created to display the name of the hot_drink on the 'hot_drink_list' box .
        :return: None
        """
        # get all the elements from 'hot_drink_kinds' table in db .
        hot_drink_kinds = cur.execute("SELECT * FROM hot_drink_kinds").fetchall()

        # insert the name of every hot_drink to 'hot_drink_list' box .
        count = 0
        for hot_drink in hot_drink_kinds:
            self.hot_drink_list.insert(count, "{}. {}".format(count+1, str(hot_drink[0])))
            count += 1

    def add_item_to_check(self):
        """
        This function will be activated when the user select one of the items in the 'hot_drink_list' box then click the
        'add' button .
        ...
        if len(self.list_detail.curselection()) > 0:
            return
        # the code under will be executed only if the user select one of the items in the 'hot_drink_list' then click 'add' btn.
        if len(self.hot_drink_list.curselection()) > 0:
            try:
                # if the user select one of the item in 'hot_drink_list' and clicked 'add' then :
                ...
                elements = cur.execute("SELECT * FROM hot_drink_kinds WHERE hot_drink_name=?", (hot_drink_name,))\
                    .fetchall()
