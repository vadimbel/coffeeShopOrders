...
class PickMeat(Toplevel):
    def __init__(self):
        Toplevel.__init__(self)
        self.geometry("650x650+650+200")
        self.title("Meat Menu")
    ...
    # create left frame and place a list with all the meat kinds in the menu .
        left_frame = Frame(self, width=200, height=70, relief=SUNKEN, borderwidth=3)
        left_frame.pack(side=LEFT, fill=Y)

        self.meat_list = Listbox(left_frame, width=35, height=30)
        self.meat_list.pack()

    # create right frame that will display all the info about every meat selected in the list box on the left frame .
        ...
        def display_values(evt):
            """
            This function will be activated when the user click on one of the items in the 'meat_list' box .
            :param evt:
            :return: None
            """
            # solve a bug : when i click on 'meat_list' then on 'list_detail' -> exception .
            if len(self.list_detail.curselection()) > 0:
                return
            # delete all elements from 'list_detail' .
            self.list_detail.delete(0, END)
            # get the selected item from the 'meat_list' box .
            meat = str(self.meat_list.get(self.meat_list.curselection()))
            ...
            elements = cur.execute("SELECT * FROM meat_kinds WHERE meat_name=?", (meat_name,)).fetchall()
        ...
        # when user click on item in the 'meat_list' box -> the details will appear in the 'list_detail' .
        self.meat_list.bind('<<ListboxSelect>>', display_values)

    def display_meat(self):
        """
        This function created to display the name of the meat on the 'meat_list' box .
        :return: None
        """
        # get all the elements from 'meat_kinds' table in db .
        meat_kinds = cur.execute("SELECT * FROM meat_kinds").fetchall()

        # insert the name of every meat to 'meat_list' box .
        count = 0
        for meat in meat_kinds:
            self.meat_list.insert(count, "{}. {}".format(count+1, str(meat[0])))
            count += 1

    def add_item_to_check(self):
        """
        This function will be activated when the user select one of the items in the 'meat_list' box then click the
        'add' button .
        ...
        if len(self.list_detail.curselection()) > 0:
            return
        # the code under will be executed only if the user select one of the items in the 'meat_list' then click 'add' btn.
        if len(self.meat_list.curselection()) > 0:
            try:
                # if the user select one of the item in 'meat_list' and clicked 'add' then :
