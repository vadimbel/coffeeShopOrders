...
class PickPasta(Toplevel):
    def __init__(self):
        Toplevel.__init__(self)
        self.geometry("650x650+650+200")
        self.title("Pasta Menu")
    ...
    # create left frame and place a list with all the pasta kinds in the menu .
        left_frame = Frame(self, width=200, height=70, relief=SUNKEN, borderwidth=3)
        left_frame.pack(side=LEFT, fill=Y)

        self.pasta_list = Listbox(left_frame, width=35, height=30)
        self.pasta_list.pack()

    # create right frame that will display all the info about every pasta selected in the list box on the left frame .
        ...
        def display_values(evt):
            """
            This function will be activated when the user click on one of the items in the 'pasta_list' box .
            :param evt:
            :return: None
            """
        # solve a bug : when i click on 'pasta_list' then on 'list_detail' -> exception .
            if len(self.list_detail.curselection()) > 0:
                return
        # delete all elements from 'list_detail' .
            self.list_detail.delete(0, END)
        # get the selected item from the 'pasta_list' box .
            pasta = str(self.pasta_list.get(self.pasta_list.curselection()))
        ...
        # get all the properties of the selected item from db in format : [( ....)] -> tuple inside a list .
            elements = cur.execute("SELECT * FROM pasta_kinds WHERE pasta_name=?", (pasta_name,)).fetchall()
        ...
        # when user click on item in the 'pasta_list' box -> the details will appear in the 'list_detail' .
        self.pasta_list.bind('<<ListboxSelect>>', display_values)


    def display_pasta(self):
        """
        This function created to display the name of the pasta on the 'pasta_list' box .
        :return: None
        """
    # get all the elements from 'pasta_kinds' table in db .
        pasta_kinds = cur.execute("SELECT * FROM pasta_kinds").fetchall()

    # insert the name of every pasta to 'pasta_list' box .
        count = 0
        for pasta in pasta_kinds:
            self.pasta_list.insert(count, "{}. {}".format(count+1, str(pasta[0])))
            count += 1

    def add_item_to_check(self):
        """
        This function will be activated when the user select one of the items in the 'pasta_list' box then click the
        'add' button .
    ...
    # solve bug : when user select one of the items in the 'list_detail' -> exception .
        if len(self.list_detail.curselection()) > 0:
            return
    # the code under will be executed only if the user select one of the items in the 'pasta_list' then click 'add' btn.
        if len(self.pasta_list.curselection()) > 0:
            try:
            # if the user select one of the item in 'pasta_list' and clicked 'add' then :
            # get the item was selected from the 'pasta_list' .
                pasta = str(self.pasta_list.get(self.pasta_list.curselection()))
        ...
        # if there any problem -> one of those two line will be executed .
                ...
                messagebox.showinfo("Error", "ops .. something wrong 1", icon='warning')
        else:
