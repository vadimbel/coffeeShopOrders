...
class PickAlcohol(Toplevel):
    def __init__(self):
        Toplevel.__init__(self)
        self.geometry("650x650+650+200")
        self.title("Alcohol Menu")
    ...
    # 'add' button at the top of the window .
        ...
        # create left frame and place a list with all the alcohol_kinds in the menu .
        left_frame = Frame(self, width=200, height=70, relief=SUNKEN, borderwidth=3)
        left_frame.pack(side=LEFT, fill=Y)

        self.alcohol_list = Listbox(left_frame, width=35, height=30)
        self.alcohol_list.pack()

        # create right frame that will display all the info about every alcohol selected in the list box on the left frame .
        ...
        self.display_alcohol()

        def display_values(evt):
            """
            This function will be activated when the user click on one of the items in the 'alcohol_list' box .
            :param evt:
            :return: None
            """
            # solve a bug : when i click on 'alcohol_list' then on 'list_detail' -> exception .
            ...
            alcohol = str(self.alcohol_list.get(self.alcohol_list.curselection()))

            # substring the name out of the selected item .
            alcohol_name = alcohol[3:]

            # get all the properties of the selected item from db in format : [( ....)] -> tuple inside a list .
            elements = cur.execute("SELECT * FROM alcohol_kinds WHERE alcohol_name=?", (alcohol_name,)).fetchall()
        ...
        # when user click on item in the 'alcohol_list' box -> the details will appear in the 'list_detail' .
        self.alcohol_list.bind('<<ListboxSelect>>', display_values)

    def display_alcohol(self):
        """
        This function created to display the name of the alcohol on the 'alcohol_list' box .
        :return: None
        """
        # get all the elements from 'cold_drink_kinds' table in db .
        alcohol_kinds = cur.execute("SELECT * FROM alcohol_kinds").fetchall()

        # insert the name of every alcohol to 'alcohol_list' box .
        count = 0
        for alcohol in alcohol_kinds:
            self.alcohol_list.insert(count, "{}. {}".format(count+1, str(alcohol[0])))
            count += 1

    def add_item_to_check(self):
        """
        This function will be activated when the user select one of the items in the 'alcohol_list' box then click the
        'add' button .
        ...
        if len(self.list_detail.curselection()) > 0:
            return
        # the code under will be executed only if the user select one of the items in the 'alcohol_list' then click 'add' btn.
        if len(self.alcohol_list.curselection()) > 0:
            try:
                # if the user select one of the item in 'alcohol_list' and clicked 'add' then :
                ...
                elements = cur.execute("SELECT * FROM alcohol_kinds WHERE alcohol_name=?", (alcohol_name,)) \
                    .fetchall()
