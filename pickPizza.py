...
class PickPizza(Toplevel):
    def __init__(self):
        Toplevel.__init__(self)
        self.geometry("650x650+650+200")
        self.title("Pizza Menu")
    ...
    # create left frame and place a list with all the pizza kinds in the menu .
        left_frame = Frame(self, width=200, height=70, relief=SUNKEN, borderwidth=3)
        left_frame.pack(side=LEFT, fill=Y)

        self.pizza_list = Listbox(left_frame, width=35, height=30)
        self.pizza_list.pack()

    # create right frame that will display all the info about every pizza selected in the list box on the left frame .
        ...
        def display_values(evt):
            """
            This function will be activated when the user click on one of the items in the 'pizza_list' box .
            :param evt:
            :return: None
            """
            # solve a bug : when i click on 'pizza_list' then on 'list_detail' -> exception .
            if len(self.list_detail.curselection()) > 0:
                return
            # delete all elements from 'list_detail' .
            self.list_detail.delete(0, END)
            # get the selected item from the 'pizza_list' box .
            pizza = str(self.pizza_list.get(self.pizza_list.curselection()))
            ...
            elements = cur.execute("SELECT * FROM pizza_kinds WHERE pizza_name=?", (pizza_name,)).fetchall()
        ...
        # when user click on item in the 'pizza_list' box -> the details will appear in the 'list_detail' .
        self.pizza_list.bind('<<ListboxSelect>>', display_values)

    def display_pizza(self):
        """
        This function created to display the name of the pizza on the 'pizza_list' box .
        :return: None
        """
        # get all the elements from 'pizza_kinds' table in db .
        pizza_kinds = cur.execute("SELECT * FROM pizza_kinds").fetchall()

        # insert the name of every pizza to 'pizza_list' box .
        count = 0
        for pizza in pizza_kinds:
            self.pizza_list.insert(count, "{}. {}".format(count+1, str(pizza[0])))
            count += 1

    def add_item_to_check(self):
        """
        This function will be activated when the user select one of the items in the 'pizza_list' box then click the
        'add' button .
        ...
        if len(self.list_detail.curselection()) > 0:
            return
        # the code under will be executed only if the user select one of the items in the 'pizza_list' then click 'add' btn.
        if len(self.pizza_list.curselection()) > 0:
            try:
                # if the user select one of the item in 'pizza_list' and clicked 'add' then :
